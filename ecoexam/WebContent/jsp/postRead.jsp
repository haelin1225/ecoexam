...
<html lang="en">
  ...
  <body>
	...
	<div class="container">
		...
		<%    		
    		if(member != null) {
    			...
    			if(member.getMemberID().equals(postVO.getMemberID())) { %>
    						<a href="postUpdateForm.eco?postNo=<%=postVO.getPostNo() %>" class="btn btn-primary">수정</a>
		...
		<table class="table">
			<tr>
				<th colspan="2"><h3><b>${post.getPostTitle()}</b> <small>| 공지사항 </small></h3></th>
			...
			<tr>
				<td colspan="2">
					<% if(postNoList.getPre_postNo() != 0) {%>
			    			<a href="postRead.eco?postNo=<%=postNoList.getPre_postNo()%>" class="btn btn-primary">« 이전</a>
			    	<% } %>
			    	<a href="boardList.eco?boardNo=${boardNo}" class="btn btn-primary">목록</a>
			    	<% if(postNoList.getNext_postNo() != 0) {%>
			    		<a href="postRead.eco?postNo=<%=postNoList.getNext_postNo()%>" class="btn btn-primary">다음 »</a>
			    	<% } %>
				</td>
			</tr>
		</table>
	...
	<script type="text/javascript">	
		function deleteConfirm(){			
			...
			if(result) location.replace("postDelete.eco?postNo=${post.getPostNo()}");
		}
